export default {
  ...
  arrow: ({ context, props }) => ({
    class: [
      ...
      'border-solid',
      {
        'border-y-[0.25rem] border-r-[0.25rem] border-l-0 border-r-[#916AE9]': context?.right || (!context?.right && !context?.left && !context?.top && !context?.bottom),
        'border-y-[0.25rem] border-l-[0.25rem] border-r-0 border-l-[#916AE9]': context?.left,
