...
import { usePublicLesson } from "~/features/course/curriculum";
...
const { data: modules, pending } = await $module.course.getModulesByCourseId(props.courseId, { asyncDataOptions: { server: false } });

const { data: lessons, pending: pendingLesson, execute: fetchLesson } = await $module.course.getLessonsByModuleId(() => modules.value?.data[currentModuleIdx.value]?.id, {
  asyncDataOptions:
    {
      immediate: false,
      server: false,
